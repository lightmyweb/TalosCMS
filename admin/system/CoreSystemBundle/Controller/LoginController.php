...
class LoginController extends Controller
{
    public function indexAction(Request $request)
    {
        ...
        return $this->render('CoreSystemBundle:Login:index.html.twig', array(
    ...
    public function motPasseOublieAction(Request $data){
        ...
        if(isset($salt)){
            $user = $em->getRepository('CoreSystemBundle:User')->findBy(
                array(
                    'salt'=>$salt,
                )
            );
            if(isset($user[0])){
                return $this->render('CoreSystemBundle:Login:mot_passe_oublie.html.twig', array(
                    'salt' => $salt,
                ));
            }
        }
        ...
        if(isset($user[0])){
            $em = $this->getDoctrine()->getManager();
            $projectTitle = $em->getRepository('CoreSystemBundle:Settings')->findOneById(1)->getTitle();
            $projectEmail = $em->getRepository('CoreSystemBundle:Settings')->findOneById(1)->getEmail();
